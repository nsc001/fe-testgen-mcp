...
export class TypeScriptAgent extends BaseAgent<Issue> {
  constructor(openai: OpenAIClient, projectContextPrompt?: string) {
    super(openai, {
      name: 'typescript',
      promptPath: getProjectPath('src/prompts/cr/typescript.md'),
      description: '审查 TypeScript 类型安全和类型边界问题',
  ...
  async execute(context: {
    ...
    try {
      ...
      logger.error('TypeScriptAgent failed', { error });
  ...
  private buildPrompt(diff: string, files: Array<{ path: string; content: string }>): string {
    ...
    return `分析以下代码变更，识别 TypeScript 相关问题：
    ...
    try {
      ...
      return parsed.map((item: any) => {
        ...
        const issue: Issue = {
          ...
          topic: CRTopic.parse('typescript'),
      ...
      logger.warn('Failed to parse TypeScriptAgent response', { response, error });
      return [];
    }
  }
}
