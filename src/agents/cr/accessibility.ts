...
export class AccessibilityAgent extends BaseAgent<Issue> {
  constructor(openai: OpenAIClient, projectContextPrompt?: string) {
    super(openai, {
      name: 'accessibility',
      promptPath: getProjectPath('src/prompts/cr/accessibility.md'),
      description: '审查无障碍访问相关问题',
  ...
  async execute(context: {
    ...
    try {
      ...
      logger.error('AccessibilityAgent failed', { error });
  ...
  private buildPrompt(diff: string, files: Array<{ path: string; content: string }>): string {
    ...
    return `分析以下代码变更，识别无障碍访问（Accessibility）相关问题：
    ...
    try {
      ...
      return parsed.map((item: any) => {
        ...
        const issue: Issue = {
          ...
          topic: CRTopic.parse('accessibility'),
