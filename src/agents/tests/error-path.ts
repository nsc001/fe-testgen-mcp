...
export class ErrorPathTestAgent extends BaseAgent<TestCase> {
  ...
  async execute(context: {
    ...
    try {
      ...
      logger.error('ErrorPathTestAgent failed', { error });
...
返回 JSON 格式的测试用例列表，scenario 应为 "error-path"。`;
  }
    ...
    try {
      ...
      return parsed.map((item: any) => {
        ...
        const test: TestCase = {
          ...
          scenario: TestScenario.parse('error-path'),
      ...
      logger.warn('Failed to parse ErrorPathTestAgent response', { response, error });
      return [];
    }
  }
}
